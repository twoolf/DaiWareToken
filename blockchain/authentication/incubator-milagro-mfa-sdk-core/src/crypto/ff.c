...
/* CLINT basic functions for Large Finite Field support */
...
/* x=y<<n */
void FF_dsucopy(BIG x[],BIG y[],int n)
{
	int i;
	for (i=0;i<n;i++)
	{
...
/* x=y */
void FF_dscopy(BIG x[],BIG y[],int n)
{
	int i;
	for (i=0;i<n;i++)
	{
...
/* x=y>>n */
void FF_sducopy(BIG x[],BIG y[],int n)
{
	int i;
	for (i=0;i<n;i++)
		BIG_copy(x[i],y[n+i]);
...
/* recursive add */
void FF_radd(BIG z[],int zp,BIG x[],int xp,BIG y[],int yp,int n)
{
	int i;
	for (i=0;i<n;i++)
...
/* recursive inc */
void FF_rinc(BIG z[],int zp,BIG y[],int yp,int n)
{
	int i;
	for (i=0;i<n;i++)
...
/* recursive sub */
void FF_rsub(BIG z[],int zp,BIG x[],int xp,BIG y[],int yp,int n)
{
	int i;
	for (i=0;i<n;i++)
...
/* recursive dec */
void FF_rdec(BIG z[],int zp,BIG y[],int yp,int n)
{
	int i;
	for (i=0;i<n;i++)
...
void FF_rnorm(BIG z[],int zp,int n)
{
	int i,trunc=0;	
	chunk carry;
	if (n<0)
...
void FF_cswap(BIG a[],BIG b[],int d,int n)
{
	int i;
	for (i=0;i<n;i++)
		BIG_cswap(a[i],b[i],d);
...
void FF_karmul(BIG z[],int zp,BIG x[],int xp,BIG y[],int yp,BIG t[],int tp,int n)
{
    int nd2;
	if (n==1)
	{
...
void FF_karsqr(BIG z[],int zp,BIG x[],int xp,BIG t[],int tp,int n)
{
	int nd2;
	if (n==1)
	{
...
void FF_karmul_lower(BIG z[],int zp,BIG x[],int xp,BIG y[],int yp,BIG t[],int tp,int n)
...
void FF_karmul_upper(BIG z[],BIG x[],BIG y[],BIG t[],int n)
...
void FF_lmul(BIG z[],BIG x[],BIG y[],int n)
{
#ifndef C99
	BIG t[2*FFLEN];
#else
...
void FF_reduce(BIG r[],BIG T[],BIG N[],BIG ND[],int n)
...
/* nesidue mod m */
void FF_nres(BIG a[],BIG m[],int n)
{
#ifndef C99
	BIG d[2*FFLEN];
#else
...
void FF_redc(BIG a[],BIG m[],BIG ND[],int n)
{
#ifndef C99
	BIG d[2*FFLEN];
#else
...
void FF_invmod2m(BIG U[],BIG a[],int n)
{
	int i;
#ifndef C99
...
void FF_modmul(BIG z[],BIG x[],BIG y[],BIG p[],BIG ND[],int n)
{
#ifndef C99
	BIG d[2*FFLEN];
#else
...
void FF_modsqr(BIG z[],BIG x[],BIG p[],BIG ND[],int n)
{
#ifndef C99
	BIG d[2*FFLEN];
#else
