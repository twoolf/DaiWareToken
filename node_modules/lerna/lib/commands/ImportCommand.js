    ...
    value: function execute(callback) {
      ...
      _async2.default.series(this.commits.map(function (sha) {
        return function (done) {
          ...
          var patch = _this2.externalExecSync("git", ["format-patch", "-1", sha, "--stdout"]).replace(/^([-+]{3} [ab])/mg, replacement).replace(/^(diff --git a)/mg, replacement).replace(/^(diff --git \S+ b)/mg, replacement).replace(/^(rename (from|to)) /mg, `\$1 ${_this2.targetDir}/`);
